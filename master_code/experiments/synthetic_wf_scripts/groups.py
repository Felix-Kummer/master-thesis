...
def run(path = ""):
    ...
    with open(path, "w") as file:
        ...
        sizes_input_mean = round(random.uniform(size_min, size_max), 2)
        input_sizes_std_dev = round(random.uniform(size_std_dev_min, size_std_dev_max), 2)
        random_sizes_input = [abs(random.gauss(sizes_input_mean, input_sizes_std_dev)) for _ in range(num_jobs)]
        ...
        random_sizes_A_out = [abs(random.gauss(sizes_A_out_mean, sizes_A_std_dev)) for _ in range(num_jobs)]

        num_B_jobs = math.floor(num_jobs / 3) + 1
        ...
        random_sizes_B_out = [abs(random.gauss(sizes_B_out_mean, sizes_B_std_dev)) for _ in range(num_B_jobs)]
        ...
        for i in range(0, num_jobs):
            ...
            file.write(f'{indent}{indent}<uses file="file_A_output{i:03d}" link="output" register="true" transfer="true" optional="false" type="data" size="{random_sizes_A_out[i]}"/>\n')

            # close job scope
            file.write(f'{indent}</job>\n')

            # update groups storage
            group_dict[round(i / 3)].add(i)

        # dependency string
        dependencies = ""

        # aggregator jobs
        for k in range(0, num_B_jobs):

            b_id = num_jobs + k

            # head
            file.write(f'{indent}<job id="ID{b_id:05d}" namespace="Montage" name="taskB" version="1.0" runtime="{abs(random.gauss(runtime_mean_A, runtime_std_dev_B))}">\n')

            # inputs (and dependencies)
            dependencies += f'{indent}<child ref="ID{b_id:05d}">\n'
            for i in group_dict[k]:
                # input
                file.write(f'{indent}{indent}<uses file="file_A_output{i:03d}" link="input" register="true" transfer="true" optional="false" type="data" size="{random_sizes_A_out[i]}"/>\n')

                # dependency
                dependencies += f'{indent}{indent}<parent ref="ID{i:05d}"/>\n'

            # close dependencies
            dependencies += f'{indent}</child>\n'


            # output
            file.write(f'{indent}{indent}<uses file="file_output{k:03d}" link="output" register="true" transfer="true" optional="false" type="data" size="{random_sizes_B_out[k]}"/>\n')
